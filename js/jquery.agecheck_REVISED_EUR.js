...
(function ($) {
  $.ageCheck = function (options) {
	...
	window['echoMonths'] = [settings.selectmonth,settings.january,settings.february,settings.march,settings.april,settings.may,settings.june,settings.july,settings.august,settings.september,settings.october,settings.november,settings.december];
    ...
    const _this = {
      ...
      buildHtml() {
		...
		const months = echoMonths;

        let html = '';
        html += '<div class="ac-overlay"></div>';
        html += '<div class="ac-container" style="background:'+settings.backgroundColor+'">';
        html += `<h2>${settings.title}</h2>`;
        html += `<p>${copy.replace('[21]', `<strong>${settings.minAge}</strong>`)}`; +'</p>';
        html += '<div class="errors"></div>';
        html += '<div class="fields"><input class="day" maxlength="2" placeholder="'+settings.dayText+'" />';
        html += '<select class="month">';
        for (let i = 0; i < months.length; i++) {
          html += `<option value="${i}">${months[i]}</option>`;
        }
        html += '</select>';
        html += '<input class="year" maxlength="4" placeholder="'+settings.yearText+'"/>';
        html += '<button class="redirect">'+settings.submitText+'</button>';	
		if (settings.useBailout === true) {
        html += '&nbsp;&nbsp;<button class="exit"><a href='+settings.bailoutURL+' target="_self">'+settings.bailout+'</button></a></div></div>';
		}
...
*/
		...
		const successMsg = `<h3>${settings.successMsg.header}</h3>`;
		
		const successDelay = settings.successDelay;
...
*/
			if (settings.siteWindow != '1') {
                window.location.replace(settings.redirectTo);
			}
			if (settings.siteWindow != '2') {
				window.open(settings.redirectTo, '_blank');
